package com.ljcx.code.shiro.realm;
...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //获得用户所有权限
        List<SysPermissionBean> permissionBeans = permissionDao.getPermissionsByUserId(account.getUserBaseBean().getId(),"ADMIN");
...
//        }
        ...
        //获取登录权限
        SysPermissionDao permissionDao = SpringUtil.getBean(SysPermissionDao.class);
        List<SysPermissionBean> permissions = permissionDao.getPermissionsByUserId(user.getUserBaseBean().getId(),"ADMIN");
        if (permissions.size() == 0) {
            throw new IncorrectCredentialsException("账号没有后台登录权限，请联系管理员");
        }
