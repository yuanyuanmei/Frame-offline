...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        //权限名称集合
        List<SysPermissionBean> permissionBeans = permissionDao.getPermissionsByUserId(account.getUserBaseBean().getId(),"APP");
        ...
        userUtil.setPermissions(account.getUserBaseBean().getUsername(),permissionBeans);
...
//        }
        ...
        //获取登录权限
        SysPermissionDao permissionDao = SpringUtil.getBean(SysPermissionDao.class);
        List<SysPermissionBean> permissions = permissionDao.getPermissionsByUserId(user.getUserBaseBean().getId(),"APP");
        if (permissions.size() == 0) {
            throw new IncorrectCredentialsException("账号没有APP登录权限，请联系管理员");
        }
