 ...
 */
    ...
    var a = {
      ...
      onClick: function(event, treeId, treeNode){
        ...
        if(typeof(oIndex) != "undefined"){
          oIndex.getInfoByTeam(treeNode);
        }
        if(typeof(oTaskList) != "undefined"){
          oTaskList.getInfoByTeam(treeNode);
        }
      ...
      input: function(){
        ...
        var selectHtml = '<div class="'+ TREE_SELECT_CLASS +' layui-unselect layui-form-select" id="'+ TREE_SELECT_ID +'">' +
            ...
            '<a class="edit-tree" onclick="treeSelectEdit()">编辑团队/成员/设备</a>' +
  ...
  /**
   * 丹妹不能点击
   * 为节点添加样式，因为tree是异步加载，所以必须在success回调中调用checkNode函数，否则无法获取生成的DOM元素
   * @param filter lay-filter属性
   * @param id 选中的id
   */
  TreeSelect.prototype.cantSelect = function(filter, id){
    var o = obj.filter(filter),
        treeObj = obj.treeObj(filter),
        treeId = o.find('.layui-treeSelect-body').attr('id'),
        node = treeObj.getNodeByParam("id", id, null);
    $("#"+node.tId+"_a").addClass('cantSelect');
    treeObj.selectNode(node);
  };
...
//20201016
/* 编辑团队 */
function treeSelectEdit(){
  layer.open({
    type: 1 //Page层类型
    , skin: 'layer-ext-myskin'
    , area: ['1310px', '735px']
    , title: ['编辑团队', 'font-size:16px']
