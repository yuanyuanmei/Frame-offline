...
class RtcClient {
  ...
  async join() {
    ...
    try {
      ...
      console.log('=======join room success======,userId:' + this.userId_ + ",roomId:" + this.roomId_ + "===========");
      //Toast.notify('进房成功！');
      this.isJoined_ = true;
    } catch (error) {
    ...
    //remote_stream    //local_stream
    addView('local_stream_' + this.userId_,'#' + this.video_el,"local_stream");
    this.localStream_.play('local_stream_' + this.userId_);
  ...
  handleEvents() {
    ...
    this.client_.on('stream-added', evt => {
      ...
      console.log('=============subscribe to this remote stream:' + userId);
    ...
    this.client_.on('stream-subscribed', evt => {
      ...
      addView('remote_stream_' + id,'#' + this.video_el,"remote_stream");

      // 在指定的 div 容器上播放音视频
      remoteStream.play('remote_stream_' + id);
      console.log('=====================stream-subscribed ID: ', 'remote-' + id + "======================");
...
function addView(id,parentId,className) {
    var _id = "video_" + oMedia._randomString(5);
    if(className == 'local_stream'){
      var _html = "        <div id=\""+_id+"\"  class=\""+ className +"\">\n" +
          "          <div class='video_div_' id=\""+id+"\"></div>\n" +
          // "           <div class=\"video-title\">聊天</div>\n" +
          "            <div class=\"video-tool\"><a id='videoClose' href='#'></a><a id='videoAudio' href='#'></a></div>"
      "        </div>"
    }else{
      var _html = "        <div id=\""+_id+"\"  class=\""+ className +"\">\n" +
          "          <div class='video_div_' id=\""+id+"\"></div>\n" +
